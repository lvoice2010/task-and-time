...
function TaskModal({ task, onClose, onUpdate, onComplete, onDelete, now }) {
  ...
  const [editM, setEditM] = useState(String(totalMins % 60));
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 16 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <button onClick={() => {
                const tm = Math.floor(taskTotal(task, now) / 60000);
                setEditH(String(Math.floor(tm / 60)));
                setEditM(String(tm % 60));
            ...
            {editingTime && (
              <div style={{ marginTop: 8, padding: 10, background: '#F8FAFC', borderRadius: 6, border: '1px solid rgba(15,23,42,0.08)', display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ fontSize: 10, color: '#64748B', fontWeight: 600, letterSpacing: '0.05em' }}>УСТАНОВИТЬ ОБЩЕЕ ВРЕМЯ</div>
                <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                  <input type="number" min="0" value={editH} onChange={e => setEditH(e.target.value)} placeholder="0"
                    style={{ ...S.input, padding: '4px 8px', fontSize: 12, width: 50 }} />
                  <span style={{ fontSize: 11, color: '#64748B' }}>ч</span>
                  <input type="number" min="0" max="59" value={editM} onChange={e => setEditM(e.target.value)} placeholder="0"
                    style={{ ...S.input, padding: '4px 8px', fontSize: 12, width: 50 }} />
                  <span style={{ fontSize: 11, color: '#64748B' }}>мин</span>
                  <span style={{ fontSize: 11, color: '#94A3B8' }}>·</span>
                  <span style={{ fontSize: 10, color: '#64748B' }}>коррекция за</span>
                  <input type="date" value={editDate} onChange={e => setEditDate(e.target.value)}
                    style={{ ...S.input, padding: '4px 8px', fontSize: 12, width: 'auto' }} />
                </div>
                {(() => {
                  const newMs = ((parseInt(editH,10)||0) * 60 + (parseInt(editM,10)||0)) * 60000;
                  const curMs = taskTotal(task, now);
                  const delta = newMs - curMs;
                  const absDelta = Math.abs(delta);
                  if (Math.abs(delta) < 60000) return null;
                  return (
                    <div className="mono" style={{ fontSize: 11, color: delta > 0 ? '#059669' : '#DC2626', fontWeight: 500 }}>
                      {delta > 0 ? '+' : '−'}{fmtDuration(absDelta)}
                    </div>
                  );
                })()}
                <button onClick={() => {
                  ...
                  const curMs = taskTotal(task, now);
                  const delta = newMs - curMs;
                  if (delta === 0) { setEditingTime(false); return; }

                  const day = parseISODate(editDate);
                  day.setHours(12, 0, 0, 0);

                  if (delta > 0) {
                    // add correction session
                    const endTs = day.getTime();
                    const startTs = endTs - delta;
                    onUpdate({ sessions: [...task.sessions, { start: startTs, end: endTs }] });
                  } else {
                    // subtract: trim sessions from end
                    let remaining = Math.abs(delta);
                    const sessions = [...task.sessions].map(s => ({...s}));
                    for (let i = sessions.length - 1; i >= 0 && remaining > 0; i--) {
                      const s = sessions[i];
                      if (s.end == null) continue; // skip active
                      const dur = s.end - s.start;
                      if (dur <= remaining) {
                        remaining -= dur;
                        sessions.splice(i, 1);
                      } else {
                        s.end = s.end - remaining;
                        remaining = 0;
                      }
                    }
                    onUpdate({ sessions });
...
function RoleBoardView({ tasks, setTasks, now, plans, activePlanId }) {
  ...
  const openAdd = (colKey) => { setAddCol(colKey); setNewTitle(''); setNewQuadrant(null); };
  const submitAdd = (ctx) => {
    const title = newTitle.trim();
    if (!title) return;
    const roleId = ctx.role || newRole;
  ...
  const colBox = (colKey, headerNode, list, width, bg, ctx) => {
   ...
   return (
    <div key={colKey}
      ...
      {headerNode}
      {addCtx && (addCol === colKey ? (
        <div style={{ marginBottom: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
          <input autoFocus value={newTitle} onChange={e => setNewTitle(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter') submitAdd(addCtx); if (e.key === 'Escape') setAddCol(null); }}
            placeholder="Глагол + результат..." style={{ ...S.input, padding: '6px 8px', fontSize: 12 }} />
          {!addCtx.role && (
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button onClick={() => setAddCol(null)} className="btn-hover" style={{ padding: '4px 8px', fontSize: 13, borderRadius: 5, color: '#64748B' }}>×</button>
          </div>
        </div>
      ) : (
        <button onClick={() => openAdd(colKey)} className="btn-hover"
  ...
  const laneHeader = (icon, title, count, accent) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8, padding: '0 2px' }}>
      <span style={{ fontSize: 13, fontWeight: 600, color: accent ? '#0284C7' : '#334155' }}>{icon} {title}</span>
      <span className="mono" style={{ fontSize: 11, color: '#94A3B8', marginLeft: 'auto' }}>{count}</span>
    </div>
  );
    ...
    return (
      <div style={{ marginBottom: 8, padding: '0 2px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: role.color }}>{role.short}</span>
          <span className="mono" style={{ fontSize: 11, color: '#94A3B8', marginLeft: 'auto' }}>{roleBacklog(role.id).length}</span>
        </div>
        {goal ? (
          <div style={{ marginTop: 4 }}>
            <div style={{ fontSize: 9, color: '#64748B', lineHeight: 1.2, wordBreak: 'break-word', maxHeight: 24, overflow: 'hidden' }}>🎯 {goal.title || 'Цель'}</div>
            <div style={{ height: 3, background: 'rgba(15,23,42,0.08)', borderRadius: 2, marginTop: 3 }}>
              <div style={{ width: `${pct == null ? 0 : pct}%`, height: '100%', background: role.color, borderRadius: 2 }} />
            </div>
          </div>
        ) : <div style={{ fontSize: 9, color: '#CBD5E1', marginTop: 3 }}>цель не задана</div>}
      </div>
    );
  };

  const divider = (text) => (
    <div style={{ flexShrink: 0, alignSelf: 'stretch', width: 2, margin: '0 6px', background: 'rgba(15,23,42,0.12)', borderRadius: 1, position: 'relative', minHeight: 120 }}>
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%) rotate(90deg)', whiteSpace: 'nowrap', fontSize: 9, fontWeight: 600, color: '#64748B', letterSpacing: '0.1em', background: '#E2E8F0', padding: '2px 8px', borderRadius: 3 }}>{text}</div>
    </div>
  );
